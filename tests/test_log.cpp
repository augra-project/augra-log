...
protected:
    void SetUp() override
    {
        ...
        tmp_dir_ = std::filesystem::temp_directory_path() /
                   ("augra_test_log_" + std::to_string(reinterpret_cast<uintptr_t>(this)));
...
TEST(ThreadSafety, ConcurrentLoggingDoesNotCrash)
{
    ...
    logger.add_sink([&](LogLevel, const char*, const std::string&) {
    ...
    EXPECT_EQ(total.load(), threads * messages_per_thread);
...
TEST(ThreadSafety, HandlerAddRemoveDuringLogging)
{
    auto& logger = Logger::instance();
    logger.clear_handlers();
    logger.clear_sinks();
    ...
    for (int i = 0; i < 100; ++i) {
        auto h = std::make_shared<StdoutHandler>();
...
TEST(ThreadSafety, ComponentLevelChangeDuringLogging)
{
    auto& logger = Logger::instance();
    logger.clear_handlers();
    logger.clear_sinks();
    ...
    std::thread log_thread([&]() {
        while (running.load(std::memory_order_relaxed)) {
            log_debug("noisy", "debug message");
            log_info("quiet", "info message");
        }
    });

    for (int i = 0; i < 200; ++i) {
        logger.set_component_level("noisy", LogLevel::Error);
        logger.clear_component_level("noisy");
    }
...
TEST(ThreadSafety, FileHandlerConcurrentWrites)
{
    auto tmp_dir = std::filesystem::temp_directory_path() /
                   "augra_test_thread_safety";
    std::filesystem::create_directories(tmp_dir);
    auto log_path = tmp_dir / "concurrent.log";

    {
        ...
        auto fh = std::make_shared<FileHandler>(log_path.string());
        ASSERT_TRUE(fh->is_open());
        ...
        for (int t = 0; t < threads; ++t) {
            workers.emplace_back([t]() {
                for (int i = 0; i < messages_per_thread; ++i) {
                    log_info("writer", "thread=%d seq=%d", t, i);
                }
            });
        }
    ...
    while (std::getline(f, line)) {
        EXPECT_TRUE(line.find("[INFO] writer:") != std::string::npos)
            << "Corrupt line: " << line;
        ++line_count;
    }

    EXPECT_EQ(line_count, 4 * 500);
